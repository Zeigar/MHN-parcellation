function parcell_aal_spec(dirname, dim_low,result_name)
...
G_support = G(G~=0);
G_support = G_support - max(G_support);
G_support = exp(15*G_support);
G(G~=0) = G_support;
...
[U,S] = eigs(G, num_clusters); % eigen decomposition
U = U*1e3;
Unorm = sqrt(sum(U.^2, 2));
mean(Unorm)
valid_idx = find(Unorm>1e-4*mean(Unorm));
fprintf('computing kmeans...\n');
labels_new(valid_idx) = kmeans(U(valid_idx,:), num_clusters, 'replicate', 5, 'Distance', 'cosine');
toc;
