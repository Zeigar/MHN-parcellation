function parcell_aal_spec_patched(dirname, dim_low, num_clusters, result_name)   % the interface updated
%% spectral clustering
% parcell_aal_spec_patched(dirname, dim_low, num_clusters, result_name)
...
toc;
save([dirname, '/eigens.mat'], 'U', 'S');

load([dirname, '/eigens.mat']);
fprintf('kmeans clustering...\n');
tic;
labels_new = kmeans(U(:,2:end), num_clusters, 'Distance', 'sqEuclidean', 'EmptyAction', 'singleton', 'Replicates', 1);
toc;
...
%%%%%%%%%%%%%%%%%%%%%%%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% load eigens;
% fprintf('DBSCAN clustering...\n');
% neighbor_map(1,:) = []; % delete the artificial row, for DBSCAN processing
% tic;
% labels_new = dbscanNeighbor(U(:,2:end), neighbor_map, -0.0005, [], @euclidDensFunc);
% toc;
...
% neighbor_map(1,:) = []; % delete the artificial row, for DBSCAN processing
% tic;
% labels_new = dbscanNeighbor([], neighbor_map, 0.87, G, @graphDensFunc);
% toc;
