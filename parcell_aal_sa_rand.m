function cluster_eval = parcell_aal_sa_rand(dirname, num_clusters, black_hole, dim_low, result_name)
...
toc;
%% construct the connectivity profile and the graph if they don't exist
if ~exist([dirname, '/graph_dimlow',num2str(dim_low), '.mat'], 'file')
    %% coarser map (for connectivity profile)
...
% random version
tic;
nii_struct = load_untouch_nii([dirname, '/mask_grey_matter_aal_90.nii.gz']);
...
for i = 1:n_seeds
    ...
    labels(seeds(i), mod(i, num_clusters)+1) = 1;
end
toc;

% labels_group = [];
% for num_clusters = 2:7
% %% kmeans initialization
% fprintf('doing kmeans clustering %d...\n', num_clusters);
% tic;
% labels = zeros(num_voxels,num_clusters); 
% ind = kmeans(conn_profile(valid_idx,:), num_clusters, 'Distance', 'cosine');
% ind = (ind-1)*num_voxels + valid_idx - 1;
% labels(ind) = 1;
% toc;
