function parcell_aal_spec_sa_patched(dirname, black_hole, dim_low, num_clusters, result_name)   % the interface updated
%% spectral clustering with hopfield network post-processing
% function parcell_aal_spec_sa_patched(dirname, black_hole, dim_low, num_clusters, result_name)
...
% initialize kmeans
fprintf('kmeans initializing...\n');
tic;
centroids = zeros(num_clusters, size(U,2)-1);
for k = 1:num_clusters
    centroids(k,:) = mean(U((labels==k),2:end), 1);
end
toc;
    
fprintf('kmeans clustering...\n');
tic;
labels_new = kmeans(U(:,2:end), num_clusters, 'Distance', 'sqEuclidean', 'EmptyAction', 'singleton', 'Replicates', 1, 'start', centroids);
toc;

labels = zeros(num_voxels,1);
labels(valid_idx) = labels_new;

%% save the temporal results for spectral clustering
...
nii_struct.hdr = hdr;
save_nii(nii_struct, [dirname,'/parcel_result_spec_sa_tmp.nii.gz']);

%% hopfield network dynamics
fprintf('doing hopfield network iterations: phase 1...\n');
labels_new = ind2vec(labels_new, num_clusters);
labels_new = labels_new*(1-black_hole) + black_hole;
