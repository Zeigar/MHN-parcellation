function parcell_aal_kmeans_sa(dirname, black_hole, dim_low,result_name)
...
[mv, mi] = max(mask, [], 2);
labels = mi .* (mv>thres);
cent = zeros(num_clusters, size(conn_profile,2));
idx = false(num_voxels,1);
idx(valid_idx-1) = true;
% compute the initial cosine centroids
for c = 1:num_clusters
    cent(c,:) = cosine_cent(conn_profile(labels==c & idx, :));
end
...
labels_new(valid_idx-1) = kmeans(conn_profile(valid_idx,:), num_clusters, 'Distance', 'cosine', 'Start', cent);
toc;
% convert index to one-out-of-p
ind = (labels_new(valid_idx-1)-1) * num_voxels + (valid_idx-1);
labels_new = zeros(num_voxels, num_clusters);
labels_new(ind) = 1;
