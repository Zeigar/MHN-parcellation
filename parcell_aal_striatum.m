function parcell_aal_striatum(dirname, black_hole, dim_low,result_name)
%% hopfield network with aal initialization
addpath('/fs/nara-scratch/qwang37/brain_data/NIFTI_20130306');
...
neighbor_dist = 1; % distance of "neighbor nodes"
num_neighbors = 0;
...
if ~exist([dirname,'/graph_dimlow',num2str(dim_low), '_nd', num2str(neighbor_dist), '.mat'], 'file')
    ...
    toc;
    save([dirname, '/graph_dimlow', num2str(dim_low), '_nd', num2str(neighbor_dist), '.mat'], 'G');
else
    load([dirname, '/graph_dimlow', num2str(dim_low), '_nd', num2str(neighbor_dist), '.mat']);
end  % endif exist graph.mat

% G_support = G(G~=0);
% % G_support = G_support - mean(G_support);
% G_support = G_support - 0.6;
% 
% G(G~=0) = G_support;
...
% num_clusters = 6;
nii_struct = load_untouch_nii([dirname, '/mask_striatum.nii.gz']);
nii_img = nii_struct.img(coord_voxel_1d);
mask_id = find(nii_img>0);
num_voxels = length(mask_id);
G = G(mask_id, mask_id);
mask = [];
for i = 1:num_clusters
    nii_struct = load_untouch_nii([dirname, '/atlas_striatum_con/striatum_con_2mm_', num2str(i), '.nii.gz']);
    nii_img = nii_struct.img(:);
    mask = [mask, nii_img(coord_voxel_1d(mask_id))];
end
...
% nii_struct = load_untouch_nii([dirname, '/mask_striatum.nii.gz']);
% nii_img = nii_struct.img(coord_voxel_1d);
% mask_id = find(nii_img > 0); % extract the striatum parts
% num_voxels = length(mask_id);
% G = G(mask_id, mask_id);
% % n_seeds = round(length(mask)/10);
% n_seeds = length(mask_id);
% perm = randperm(length(mask_id));
% seeds = perm(1:n_seeds);
% labels = zeros(num_voxels, num_clusters);
% for i = 1:n_seeds
%     cc = floor(rand()*num_clusters) + 1;
%     labels(seeds(i), cc) = 1;
% end
...
for iter = 1:num_iter
    ...
    kept_ind = find(mv > 0);
    ind = (ind(kept_ind)-1)*num_voxels + (1:length(ind))';
...
for i = 1:num_voxels
    view_field(coord(mask_id(i),1)+1, coord(mask_id(i),2)+1, coord(mask_id(i),3)+1) = labels_new(i);
end
view_field = view_field(end:-1:1,:,:); % seems that the left-right has been reversed
...
view_field = zeros(field_bound(1), field_bound(2), field_bound(3));
for i = 1:num_voxels
    view_field(coord(mask_id(i),1)+1, coord(mask_id(i),2)+1, coord(mask_id(i),3)+1) = labels(i);
end
view_field = view_field(end:-1:1,:,:);
view_field_nii = make_nii(view_field);
view_field_nii.hdr.dime.pixdim = nii_ref.hdr.dime.pixdim;
save_nii(view_field_nii, [dirname, '/parcel_result_start_striatum_con.nii.gz']);
